...
class HomeController extends Controller
{
    ...
    public function indexCached() : View
    {
        $sortedPosts = Cache::remember('sortedPosts', 600, function () {
            return Sorting::with('post.tags')->where('location', 1)->orderBy('order', 'asc')->get();
        });

        $headlineRightNews = Cache::remember('headlineRightNews', 600, function () {
            return Sorting::with('post.tags')->where('location', 2)->orderBy('order', 'asc')->get();
        });

        $editorsPicks = Cache::remember('editorsPicks', 600, function () {
            return Post::with('tags')->where('is_active', 1)->orderBy('created_at', 'desc')->get();
        });

        $editorsPicksBottom = Cache::remember('editorsPicksBottom', 600, function () {
            return Post::with('tags')->where('is_active', 1)->orderBy('created_at', 'desc')->skip(1)->take(6)->get();
        });

        $newsPosts = Cache::remember('newsPosts', 600, function () {
            return Post::with('tags')->where('location', 0)->whereHas('categories', function ($query) {
                $query->where('name', "News");
            })
            ->orderBy('created_at', 'desc')
            ->take(3)->get();
        ...
        $topStories = Cache::remember('topStories', 600, function () {
            return Post::with('tags')->with('user')->where('is_active', 1)->orderBy('created_at', 'desc')->get();
        });

        $worldInPictures = Cache::remember('worldInPictures', 600, function () {
            return Post::with('tags')->where('is_active', 1)->orderBy('created_at', 'desc')->take(5)->get();
        });



        return view('pages.home', compact('sortedPosts', 'headlineRightNews',
                    'editorsPicks','editorsPicksBottom', 'newsPosts', 'sportsPosts', 'topStoryIndex', 'worldInPictures', 'topStories', 'travelPosts', 'soundsPosts'));
    }
}
